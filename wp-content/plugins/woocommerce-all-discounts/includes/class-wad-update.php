...
class Woocommerce_All_Discount_License {
	...
	/**
	 * Woocommerce_All_Discount_License constructor.
	 */
     ...
     * If you want add an interrupt in your plugin or theme simply can use this static method: Woocommerce_All_Discount_License::is_activated
     * This will return true or false for license status
	 * @return bool
	 */
	public static function is_activated() {
		$private_session = get_option('Woocommerce_All_Discount_register_license_settings');
		$now = json_decode(get_option($private_session));
		if( empty($now) ) {
			return false;
		} elseif($now->action != 1) {
	 ...
	 * @param $settings
	 *
	 * @return null|Woocommerce_All_Discount_License
	 */
...
add_action('init', 'Woocommerce_All_Discount_init');
/**
 * Initialize function for class and hook it to wordpress init action
 */
function Woocommerce_All_Discount_init() {
	$settings = [
		'name'          => '',
		'slug'          => 'Woocommerce_All_Discount_woo',
		'parent_slug'   => 'edit.php?post_type=o-discount', // Read this: https://developer.wordpress.org/reference/functions/add_submenu_page/#parameters
		'text_domain'   => 'wad',
		'product_token' => 'e6ae344e-8894-4ec2-9045-08f54ba4befe', // Get it from here: https://zhaket.com/dashboard/licenses/
		'option_name'   => 'Woocommerce_All_Discount_register_license_settings'
	];
	Woocommerce_All_Discount_License::instance($settings);
}
