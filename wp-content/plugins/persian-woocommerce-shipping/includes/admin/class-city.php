...
class PWS_City {
	...
	public function nabik_edit_state_callback() {
		...
		if ( isset( $_POST['submit'], $_POST['taxonomy'], $_POST['tag_ID'], $_POST['action'], $_POST['term_meta'] ) ) {
			...
			$message = '<div id="message" class="updated"><p><strong>تنظیمات با موفقیت ذخیره شدند.</strong></p></div>';
		}
		...
		?>
		<div class="wrap">
			<h1>ویرایش <?php echo $state_term->name; ?></h1>
			<?php echo $message; ?>
			...
			<form method="post" action="" class="validate">
				...
				<table class="widefat" cellspacing="0">
					...
					<?php foreach ( $terms as $term ) {
						...
						echo "<tr><td>{$indent}{$term->name}</td>";
		...
		<script>
            jQuery(document).ready(function ($) {
                ...
                $("input[type=text]").attr('placeholder', '<?php echo html_entity_decode( get_woocommerce_currency_symbol() ); ?>');
            });
		</script>
		<?php
	}
