...
(function($){
	...
	function __add_events() {
		...
		$(document).on( 'svx-fields-on-screen', function(e) {
			...
			if ( $('#svx-duplicate-filter').length==0 && $('#_filter_preset_manager').length>0 ) {
				$('#_filter_preset_manager').after((svx.language?'<span class="svx-button svx-disabled">'+svx.language.toUpperCase()+'</span>':'')+'<span id="svx-duplicate-filter" class="svx-button">نسخه کپی</span><span id="svx-new-filter" class="svx-button">جدید</span><span id="svx-delete-filter" class="svx-button">حذف</span>');
			}
	...
	function main_settings(e) {
		...
		var tmplData = {
			...
			desc: svx.sections[e.target.dataset.id].desc+' &rarr; <a href="'+svx.doc.url+'">به کمک نیاز دارید؟ میتوانید از ما کمک بگیرید.</a>',
			settings: settings
		};
	...
	function _options_export(e) {
		...
		$.when( svx_ajax(settings) ).done( function(response) {
			...
			message_alert( 'خروجی ساخته شد!', 'notification', false );
	...
	function _options_import(e) {
		...
		if (e.target && !e.target.matches('.svx-active')) {
			if ( $('#svx-plugin-options').length==0 ) {
				$('#svx-export').closest('.svx-option-wrapper').prepend('<textarea id="svx-plugin-options" placeholder="خروجی را اینجا جایگذاری کرده و دوباره وارد کردن را کلیک کنید..." />');
			}
			else {
		...
		if ( typeof importVals === 'string' && importVals.substr(0,1) == '{' ) {
			...
			$.when( svx_ajax(settings) ).done( function(response) {
				...
				message_alert( 'وارد شد!', 'notification', false );
			...
			message_alert( 'اطلاعات وارد شده صحیح نیست!', 'notification', false );
	...
	function _options_backup(e) {
		...
		$.when( svx_ajax(settings) ).done( function(response) {
			svx.backup = 'just now';
			message_alert( 'پشتیبان گرفته شد!', 'notification', false );
		} );
	}

	function _options_restore(e) {
		if ( typeof svx.backup == 'undefined' ) {
			alert('پشتیبانی وجود ندارد!');
			return false;
		}

		if ( !confirm('نسخه پشتیبان '+svx.backup+' برای بازگردانی موجود است! مطمئن هستید?') ) {
			return false;
		}
		...
		$.when( svx_ajax(settings) ).done( function(response) {
			message_alert( 'پشتیبان بازگردانی شد!', 'notification', false );
	...
	function _options_reset(e) {
		if ( !confirm('این عملیات کلیه تنظیمات را به حالت اول برمیگرداند. اما پشتیبان ها حذف نمیشوند. مطمئن هستید?') ) {
			return false;
		}
		...
		$.when( svx_ajax(settings) ).done( function(response) {
			message_alert( 'تنظیمات به حالت پیشفرض بازگشت!', 'notification', false );
	...
	function __make_function_options(p) {
		switch (p[1]) {
			case '__make_presets':
				var op = {}
				if ( u(p[2]) == 'template' ) {
					op._do_not = '** جایگزین نکن';
					op._hide = '** المنت مخفی';
				}
				if ( u(p[2]) == 'has_none' ) {
					op[''] = 'هیچ';
				}
				op.default = 'پیشفرض (default)';
	...
	function make_control_options(e,f) {
		...
		if ( f.options.toString().substr(0,5) == 'ajax:' ) {
			...
			return '<option value="svx_load_'+id+'" selected="selected">بارگذاری..</option>';
		}
	...
	function add_list_items(e,f) {
		...
		$.each( f.val, function(b,c) {
			...
			if ( typeof c !== 'undefined' ) {
				...
				if ( tmplData.title == 'تنظیم نشده' && u(c.default_name) ) {
	...
	function _ajax_save_settings() {
		...
		$.when( svx_ajax(settings) ).done( function(response) {
			message_alert( 'ذخیره شد!', 'notification', false );
		} );
	...
	function include_customizer_call(e,f) {
		...
		$('#svx-include-customizer-terms').append('<select id="svx-include-selectize" class="svx-selectize" multiple placeholder="برای انتخاب کلیک کنید"></select>');
		...
		for ( var i=x; i<Math.floor(x/5)*5+5; i++) {
			...
			if ( g !== false ) {
				...
				var tmplData = {
					...
					slug: u(g.slug),
					name: u(g.name),
					title: _set_term_name(u(g.default_name)?g.default_name:u(g.name)?g.name:'تنظیم نشده'),
					taxonomy: u(f.taxonomy),
