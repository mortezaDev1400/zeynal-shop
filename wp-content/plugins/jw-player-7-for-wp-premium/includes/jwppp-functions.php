...
function jwppp_single_video_box( $post_id, $number ) {
	...
	if ( 1 === intval( $number ) && ! $dashboard_player ) {
		...
		$output .= 'با استفاده از این کد کوتاه یک لیست پخش از ویدیوهای خود اضافه کنید: <code style="display:inline-block;color:#fff;background:none;">[jwp-video n="' . jwppp_videos_string( $post_id ) . '"]</code>';
