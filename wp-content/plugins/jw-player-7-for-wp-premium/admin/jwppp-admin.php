...
function jwppp_register_js_menu() {
	...
	wp_enqueue_style( 'jwppp-admin-style-rtl', plugin_dir_url( __DIR__ ) . 'css/jwppp-admin-style-rtl.css' );
}
...
function jwppp_options() {
	...
	if(jwp_license::is_activated() ===true){
	/*Start page template*/
	echo '<div class="wrap">';
	...
	<h2 id="jwppp-admin-menu" class="nav-tab-wrapper">
		<a href="#" data-link="jwppp-settings" class="nav-tab nav-tab-active" onclick="return false;"><?php esc_html_e( 'تنظیمات عمومی' , 'jwppp' ); ?></a>
		<?php if ( ! $dashboard_player ) { ?>
			<a href="#" data-link="jwppp-skin" class="nav-tab" onclick="return false;"><?php esc_html_e( 'Skin', 'jwppp' ); ?></a>
			<a href="#" data-link="jwppp-subtitles" class="nav-tab" onclick="return false;"><?php esc_html_e( 'تنظیمات زیرنویس' , 'jwppp' ); ?></a>
			<a href="#" data-link="jwppp-social" class="nav-tab" onclick="return false;"><?php esc_html_e( 'تنظیمات اشتراک گذاری', 'jwppp' ); ?></a>    
			<a href="#" data-link="jwppp-related" class="nav-tab" onclick="return false;"><?php esc_html_e( 'تنظیمات پست های مرتبط', 'jwppp' ); ?></a>
		<?php } else { ?>
	 ...
	 <div name="jwppp-settings" id="jwppp-settings" class="jwppp-admin" style="display: block;">
		...
		if ( ! $dashboard_player ) {
			...
			echo 'value="1" /><span class="pt-note">' . esc_html( __( 'Use the post thumbnail', 'jwppp' ) ).'</span>';
			...
			echo 'placeholder="' . esc_attr( __( 'لیست پخش', 'jwppp' ) ) . '" value="' . esc_attr( $jwppp_playlist_tooltip ) . '" />';
...
?>  
	...
	<div name="jwppp-skin" id="jwppp-skin" class="jwppp-admin" style="display: none;">
		<div class="jwppp-alert"><?php esc_html_e( 'تنظیمات سفارشی‌سازی پوسته به نسخه JW Player در حال استفاده بستگی دارد. لطفا ابتدا "آدرس کتابخانه پخش کننده" را در بخش تنظیمات عمومی افزونه اضافه کنید و سپس به این قسمت مراجعه کنید.', 'jwppp' ); ?></div>		
	</div>
