...
class jwp_license {
	...
	public function menu_content() {
        ...
        <div class="wrap wpmlr_wrap" data-show="<?php echo $data_show ?>">
			...
			<?php if( isset($now) && !empty($now) ): ?>
                <p><?php _e('شما قبلاً کلید لایسنس خود را ثبت کرده اید. برای تایید مجدد آن، می توانید از این فرم استفاده کنید.', $this->text_domain); ?></p>
                <div class="current_license">
                    <span class="current_label"><?php _e('کلید لایسنس شما :', $this->text_domain); ?></span>
                    ...
                    <div class="keys">
                        ...
                        <a href="#" class="button zhk_guard_new_key"><?php _e('حذف و ثبت یک لایسنس دیگر', $this->text_domain); ?></a>
                    </div>

                    <form action="#" method="post" class="register_version_form">
                        <input type="text" class="license_key" placeholder="<?php _e('کلید لایسنس جدید', $this->text_domain); ?>">
			...
			<?php else: ?>
                <p><?php _e('لطفاً کلید لایسنس خود را در کادر زیر وارد کنید تا بتوانید از افزونه استفاده کنید.', $this->text_domain); ?></p>
                <form action="#" method="post" class="register_version_form">
                    <input type="text" class="license_key" placeholder="<?php _e('کلید لایسنس خود را وارد کنید', $this->text_domain); ?>">
                    <button class="button button-primary"><?php _e('Register version', $this->text_domain); ?></button>
                    <div class="result">
                        <div class="spinner"><?php _e('لطفا صبر کنید ...', $this->text_domain); ?></div>
                        <div class="result_text"></div>
		...
		<?php elseif( $now->action != 1 ): ?>
            <div class="notice notice-error">
                <p>
                    ...
                    <a href="<?php echo admin_url( 'admin.php?page='.$this->slug ); ?>" class="button button-primary"><?php _e('بررسی', $this->text_domain); ?></a>
                </p>
            </div>
		<?php endif; ?>
		<?php
     ...
     * If you want add an interrupt in your plugin or theme simply can use this static method: jwp_license::is_activated
...
add_action('init', 'samandon_jwp_guard_init');
/**
 * Initialize function for class and hook it to wordpress init action
 */
function samandon_jwp_guard_init() {
	$settings = [
		'name'          => 'JW Player for WordPress - Premium',
		'slug'          => 'zhk_guard_register',
		'parent_slug'   => 'jw-player-for-wp', 
		'text_domain'   => 'jwppp',
		'product_token' => 'ceeb3ac7-f732-4666-87c2-9d1a996f92c1', 
		'option_name'   => 'zhk_guard_register_settings'
	];
	jwp_license::instance($settings);
}