...
class THWCFE_Public_MyAccount extends THWCFE_Public {
	...
	private function enqueue_scripts($suffix, $jquery_version, $in_footer) {
		...
		$deps = array();
		wp_register_script('rtl-ui-datepicker', THWCFE_ASSETS_URL_PUBLIC.'js/rtl-ui-datepicker.js', array('jquery'), $in_footer);
		if( apply_filters( 'thwcfe_include_jquery_ui_i18n', TRUE ) ) {
			...
			wp_register_script('rtl-ui-datepicker', THWCFE_ASSETS_URL_PUBLIC.'js/rtl-ui-datepicker.js', array('jquery'), $in_footer);
			wp_register_script('jquery-ui-i18n', THWCFE_ASSETS_URL_PUBLIC.'js/jquery-ui-i18n.min.js', array('jquery'), $in_footer);

			$deps[] = 'jquery-ui-i18n';
		}else{
			$deps[] = 'jquery';
			$deps[] = 'rtl-ui-datepicker';
		}
					...
					if(apply_filters('thwcfe_ignore_custom_fields_in_address_to_edit', false)) {
						...
						if(apply_filters('thwcfe_edit_address_ignore_row_split', true)){
							if(isset($field['class']) && is_array($field['class'])){
								...
								$field['class'][] = 'به شکل ردیف گسترده';
							}
						}
