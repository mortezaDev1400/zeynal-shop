	...
	parseDate: function( format, value, settings ) {
		...
		var iFormat, dim, extra,
			...
			shortYearCutoff = ( typeof shortYearCutoffTemp !== "string" ? shortYearCutoffTemp :
				new this.CDate().getFullYear() % 100 + parseInt( shortYearCutoffTemp, 10 ) ),
		...
		date = this._daylightSavingAdjust( new this.CDate( year, month - 1, day ) );
...
var Date = this.CDate;
	...
	_setDate: function( inst, date, noChange ) {
		...
		var clear = !date,
			...
			newDate = this._restrictMinMax( inst, this._determineDate( inst, date, new this.CDate() ) );
  ...
  this.CDate = this._get(inst, "calendar");
	...
	_generateHTML: function( inst ) {
		var maxDraw, prevText, prev, nextText, next, currentText, gotoDate,
			...
			tempDate = new this.CDate(),
			today = this._daylightSavingAdjust(
				new this.CDate( tempDate.getFullYear(), tempDate.getMonth(), tempDate.getDate() ) ), // clear time
			isRTL = this._get( inst, "isRTL" ),
		...
		if ( maxDate ) {
			maxDraw = this._daylightSavingAdjust( new this.CDate( maxDate.getFullYear(),
		...
		for ( row = 0; row < numMonths[ 0 ]; row++ ) {
			...
			for ( col = 0; col < numMonths[ 1 ]; col++ ) {
				selectedDate = this._daylightSavingAdjust( new this.CDate( drawYear, drawMonth, inst.selectedDay ) );
				...
				printDate = this._daylightSavingAdjust( new this.CDate( drawYear, drawMonth, 1 - leadDays ) );
	...
	_generateMonthYearHeader: function( inst, drawMonth, drawYear, minDate, maxDate,
		...
		if ( !inst.yearshtml ) {
			...
			if ( secondary || !changeYear ) {
				...
				thisYear = new this.CDate().getFullYear();
	...
	_adjustInstDate: function( inst, offset, period ) {
		var year = inst.selectedYear + ( period === "Y" ? offset : 0 ),
			...
			date = this._restrictMinMax( inst, this._daylightSavingAdjust( new this.CDate( year, month, day ) ) );
	...
	_canAdjustMonth: function( inst, offset, curYear, curMonth ) {
		var numMonths = this._getNumberOfMonths( inst ),
			date = this._daylightSavingAdjust( new this.CDate( curYear,
...
this.CDate = this._get(inst, "calendar");
	...
	_formatDate: function( inst, day, month, year ) {
		...
		var date = ( day ? ( typeof day === "object" ? 
    day : this._daylightSavingAdjust( new this.CDate( year, month, day ) ) ) :
