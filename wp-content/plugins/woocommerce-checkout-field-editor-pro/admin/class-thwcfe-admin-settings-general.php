...
class THWCFE_Admin_Settings_General extends THWCFE_Admin_Settings {
	...
	public function render_sections() {
		...
		foreach( $sections as $name => $section ){
			...
			if(THWCFE_Utils_Section::is_custom_section($section)){
                ...
                <li>
                    ...
                    <span class='s_edit_btn dashicons dashicons-edit tips' data-tip='<?php THWCFE_i18n::et('ویرایش بخش'); ?>'  
					onclick='thwcfeOpenEditSectionForm(<?php echo $props_json; ?>)'></span>
                </li>
                <li>
					<span class="s_copy_btn dashicons dashicons-admin-page tips" data-tip="<?php THWCFE_i18n::et('کپی بخش'); ?>" onclick='thwcfeOpenCopySectionForm(<?php echo $props_json; ?>)'></span>
				</li>
				<li>
                    <form method="post" action="">
                        <input type="hidden" name="s_action" value="remove" />
                        <input type="hidden" name="i_name" value="<?php echo $name; ?>" />
                        <span class='s_delete_btn dashicons dashicons-no tips color-red' data-tip='<?php THWCFE_i18n::et('حذف بخش'); ?>' onclick='thwcfeRemoveSection(this)'></span>
					</form>
                </li>
                <?php
			}
		...
		echo '<li><a href="javascript:thwcfeOpenNewSectionForm()" class="btn btn-tiny btn-primary ml-30">+ '. THWCFE_i18n::t('افزودن بخش جدید') .'</a></li>';
		echo '</ul>';		
	...
	private function create_section(){
		...
		if($result == true){
			return $this->print_notices('بخش جدید با موفقیت اضافه شد.', 'بروزرسانی شد', true);
		}else{
			return $this->print_notices('بخش جدید به دلیل خطا اضافه نشد.', 'خطا', true);
		}
	}
	...
	private function remove_section(){
		...
		if($section_name){	
			...
			if ($result == true) {
				return $this->print_notices('بخش با موفقیت حذف شد.', 'updated', true);
			} else {
	...
	private function render_fields_table_heading(){
		...
		<th class="name"><?php THWCFE_i18n::et('نام'); ?></th>
		<th class="type"><?php THWCFE_i18n::et('نوع'); ?></th>
		<th class="label"><?php THWCFE_i18n::et('برچسب'); ?></th>
		<th class="placeholder"><?php THWCFE_i18n::et('مکان نگهدارنده'); ?></th>
		<th class="validate"><?php THWCFE_i18n::et('اعتبارسنجی ها'); ?></th>
        <th class="status"><?php THWCFE_i18n::et('ضروری'); ?></th>
		<th class="status"><?php THWCFE_i18n::et('فعال شد'); ?></th>	
        <th class="actions align-center"><?php THWCFE_i18n::et('اقدامات'); ?></th>	
        <?php
	}
		...
		?>
            <th colspan="5">
                <button type="button" class="btn btn-small btn-primary" onclick="thwcfeOpenNewFieldForm('<?php echo $section->get_property('name'); ?>')">
                    <?php THWCFE_i18n::et('+ افزودن فیلد'); ?>
                </button>
...
<!--                 <input type="submit" name="reset_fields" class="btn btn-small" value="<?php THWCFE_i18n::et('Reset to default fields') ?>" style="float:right; margin-right: 5px;" 
				...
				<select name="bulk_action_options" style="float:right; margin-right: 5px; height:32px" onchange="thwcfeBulkActionListner(this)">
					<option value="">اقدامات</option>
					<option value="reset_section">تنظیم مجدد این بخش</option>
					<option value="reset_all_sections">تنظیم مجدد همه بخش ها</option>
					<option value="export_section">بخش برون ریزی</option>
					<option value="export_fields">برون ریزی زمینه ها</option>
					<option value="import_fields_section">قسمتها / بخش را وارد کنید</option>
				</select>
					...
					?>
						<tr class="row_<?php echo $i; echo($is_enabled === 1 ? '' : ' thpladmin-disabled') ?>">
							...
							<td class="td_actions" align="center">
								...
								<?php }else{ ?>
									<span class="f_edit_btn dashicons dashicons-edit tips" data-tip="<?php THWCFE_i18n::et('ویرایش فیلد'); ?>"  
								...
								<?php }else{ ?>
									<span class="f_copy_btn dashicons dashicons-admin-page tips" data-tip="<?php THWCFE_i18n::et('کپی فیلد'); ?>"  
	...
	private function save_or_update_field($section, $action) {
		try {
			...
			if(is_object($field) && isset($field->name) && !empty($field->name) && isset($field->type) && !empty($field->type)){
				...
				if($result == true){
					$this->print_notices('تغییرات شما ذخیره شد.', 'updated');
